...
We use a publicly available dataset from Kaggle, titled *NBA Boxscore - Season 2023 / 2024*\footnote{\url{https://www.kaggle.com/datasets/albi9702/nba-boxscore-season-2023-2024?resource=download}}, which contains box score statistics for every player-game combination from the 2023-2024 NBA regular season. The full dataset includes 32,385 observations and 30 variables. After filtering for starters, our working dataset contains 12,300 observations. This step ensures we are modeling players with consistent playing time, which improves model reliability and interpretability.
...
```{r, echo=FALSE, fig.cap="4.1.1", out.width = '45%', fig.align='center', fig.show='hold'}
knitr::include_graphics(c("./images/4.1.1.png", "./images/3.4b.png"))
```
...
```{r, echo = FALSE, warning = FALSE, message = FALSE}
print(paste("ESS of model 1,  Max is: " , round(effectiveSize(fgm_sim_max)), "Median is: ", round(effectiveSize(fgm_sim_median)), "Mean is: ", round(effectiveSize(fgm_sim_mean))))
```
...
```{r, echo=FALSE, fig.cap="4.2.2a", out.width = '48%', fig.align='center', fig.show='hold'}
knitr::include_graphics(c("./images/4.2.2a.png", "./images/4.2.1c.png"))
```
...
df = data.frame(FixedBF = BFmodel_1,
PoissonBF = BFmodel_2,
NegBinomBF = BFmodel_3)
...
The model has no fitting concerns if $p_{ppp}$ = P(T($y_{rep(s)}$) $\geq$ T(y) \| y) is near 0.5.

We approximate $$ p(\tilde{y}| y_{i,1:J,k} ) = \sum_n\int p(\tilde{y}|n_{ijk},p_{ik},y_{i,1:j,k})p(n_{ijk},p_{ik}|y_{i,1:j,k})dp$$ with posterior samples $$(1/S)\sum_s p(\tilde{y}|n_{ijk}^{(s)},p_{ik}^{(s)},y_{i,1:J,k})$$
...
```{r, echo=FALSE, fig.cap="4.3.3", out.width = '45%', fig.align='center', fig.show='hold'}
knitr::include_graphics(c("./images/4.3.3a.png", "./images/4.3.3b.png"))
```
...
we found that the mena and variance weren't always equal
...
The marginal likelihood under Model 1 can be expressed as:

$$
\begin{aligned}
  p(y \mid M_1) &= \sum_{\{n\}} \int_{p_i}  \prod_j \binom{n_{ijk}}{y_{ijk}} p_{ik}^{y_{ijk}} (1 - p_{ik})^{n_{ijk} - y_{ijk}} \\
  &\quad \cdot \frac{\Gamma(a_i)\Gamma(b_i)}{\Gamma(a_i + b_i)} p_i^{a_i - 1} (1 - p_i)^{b_i - 1} dp_i
\end{aligned}
$$
...
\begin{aligned}
  ...
  &\propto \prod_{j} \binom{n_i}{y_{ijk}}{p_{ik}}^{y_{ijk}}
  (1-p_{ik})^{n_i-y{ijk}}
  \binom{n_{ijk}+r_{i}-1}{n_{ijk}} (1-\theta_i)^{n_{ijk}} \theta_i^{r_i} \\
  &\propto (\prod_j\frac{(n_{ijk}+r_i-1)!}{(n_{ijk}-y_{ijk})!}) ((1-\theta_i)(1-p_{ik}))^{\sum_{j}n_{ijk}}
\end{aligned}
$$
...
The marginal likelihood under Model 3 can be expressed as:

$$
\begin{aligned}
  p(y \mid M_2) &= \sum_{\{n\}} \sum_{\{r\}}\int_{p_i}\int_{\theta_i}  \prod_j \binom{n_{ijk}}{y_{ijk}} p_{ik}^{y_{ijk}} (1 - p_{ik})^{n_{ijk} - y_{ijk}} \\
  &\quad \cdot \frac{\Gamma(a_i)\Gamma(b_i)}{\Gamma(a_i + b_i)} p_i^{a_i - 1} (1 - p_i)^{b_i - 1} \\
  &\quad \cdot \prod_j \frac{\lambda_i^{n_{ijk}} e^{-\lambda_i}}{n_{ijk}!} \cdot \lambda_i^{-1/2} \\
  &\quad \cdot \binom{n_{ijk}+r_{i}-1}{n_{ijk}} (1-\theta_i)^{n_{ijk}} \theta_i^{r_i} \\ &\quad \cdot \sqrt{r_i/\theta_i^2(1-\theta_i)}
   d\theta_i \, dp_i
\end{aligned}
$$
...
```{r, echo=FALSE, fig.cap="3.3.3", out.height = '25%', fig.align='center'}
